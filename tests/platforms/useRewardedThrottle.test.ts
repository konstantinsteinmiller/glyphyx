...
describe('useRewardedThrottle', () => {
  ...
  it('persists history inside the glyphyx_state blob under the internal-key field', async () => {
    const mod = await importThrottle()
    ...
    // history is a sub-field of `glyphyx_state`.
    expect(window.localStorage.getItem(STORAGE_KEY)).toBeNull()
    const blob = JSON.parse(window.localStorage.getItem('glyphyx_state') || '{}')
  ...
  it('restores history from localStorage on module load', async () => {
    ...
    window.localStorage.setItem('glyphyx_state', JSON.stringify({ [STORAGE_KEY]: seed }))
    const mod = await importThrottle()
