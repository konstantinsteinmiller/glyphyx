...
describe('GlitchStrategy (isGlitch guard)', () => {
  ...
  it('hydrates local state from the Glitch save list', async () => {
    const payload = { gx_coins: '99', gx_best_stage: '[1,2]' }
    ...
    expect(window.localStorage.getItem('gx_coins')).toBe('99')
    expect(window.localStorage.getItem('gx_best_stage')).toBe('[1,2]')
  ...
  it('uploads a bundled Base64 payload with a SHA-256 checksum on write', async () => {
    ...
    window.localStorage.setItem('gx_coins', '10')
    ...
    expect(decoded.gx_coins).toBe('10')
  ...
  it('handles 409 conflicts by calling /resolve with the default use_client policy', async () => {
    ...
    window.localStorage.setItem('gx_coins', '77')
    await manager.flush()
  ...
  it('respects a custom keep_server conflict resolver and refreshes from the server', async () => {
    const remotePayload = { gx_coins: '200' }
    const fetchImpl = makeFetch({
      [`GET ${savesUrl}`]: () =>
        jsonResponse({
          data: [
            {
              ...
              slot_index: 0,
              version: 1,
              payload: base64Encode(JSON.stringify({ gx_coins: '1' }))
            }
          ]
        }),
      [`POST ${savesUrl}`]: () =>
    ...
    expect(window.localStorage.getItem('gx_coins')).toBe('1')

    window.localStorage.setItem('gx_coins', '999')
    await manager.flush()

    expect(window.localStorage.getItem('gx_coins')).toBe('200')
  ...
  it('degrades to a pure local mirror when list fetch fails', async () => {
    ...
    window.localStorage.setItem('gx_best_node', '5')
    ...
    window.localStorage.setItem('gx_coins', '3')
    expect(window.localStorage.getItem('gx_coins')).toBe('3')
  ...
  it('excludes internal bookkeeping keys from the payload', async () => {
    ...
    window.localStorage.setItem('gx_coins', '1')
    await manager.flush()
    ...
    expect(Object.keys(decoded)).toEqual(['__save_meta__', 'gx_coins'])
  })
  ...
  it('flips to guest-blocked mode on 403 and suppresses further uploads', async () => {
    ...
    window.localStorage.setItem('gx_coins', '5')
    await manager.flush()
    ...
    expect(window.localStorage.getItem('gx_coins')).toBe('5')
  })
  ...
  it('flips to guest-blocked on 403 during upload, cancelling pending flushes', async () => {
    ...
    window.localStorage.setItem('gx_coins', '6')
    await manager.flush()
    expect(postCount).toBe(1)
  })
  ...
  it('skips upload when the encoded payload would exceed 10MB', async () => {
    ...
    window.localStorage.setItem('gx_huge_test', bigValue)
    await manager.flush()
  ...
  it('does not round-trip its own version/save-id keys through the payload', async () => {
    ...
    expect(decoded.gx_coins).toBe('1')
  })
})
