...
describe('CrazyGamesStrategy (per-key)', () => {
  ...
  it('hydrates localStorage from the SDK manifest at boot', async () => {
    const data = makeFakeData({
      [MANIFEST_KEY]: JSON.stringify([SAVE_KEYS.COINS, SAVE_KEYS.BEST_NODE]),
      [SAVE_KEYS.COINS]: '150',
    ...
    expect(window.localStorage.getItem(SAVE_KEYS.BEST_NODE)).toBe('3')
  })
  ...
  it('mirrors subsequent writes to the SDK after a debounce, sorted', async () => {
    ...
    window.localStorage.setItem(SAVE_KEYS.BEST_NODE, '2')
    ...
    expect(writes).toContainEqual([SAVE_KEYS.BEST_NODE, '2'])
    ...
    expect(manifest).toContain(SAVE_KEYS.BEST_NODE)
  })
  ...
  it('META blob rides along with each flush so the next hydrate can merge', async () => {
    ...
    window.localStorage.setItem(SAVE_KEYS.BEST_NODE, '4')
    await vi.runAllTimersAsync()
  ...
  it('pushes settings keys present in local but missing from sdk.data', async () => {
    ...
    window.localStorage.setItem('gx_user_sound_volume', '0.7')
    window.localStorage.setItem('gx_user_music_volume', '0.6')
    window.localStorage.setItem('gx_user_language', 'en')
    window.localStorage.setItem('gx_user_difficulty', 'medium')
    ...
    expect(data.setItem).toHaveBeenCalledWith('gx_user_sound_volume', '0.7')
    expect(data.setItem).toHaveBeenCalledWith('gx_user_music_volume', '0.6')
    expect(data.setItem).toHaveBeenCalledWith('gx_user_language', 'en')
    expect(data.setItem).toHaveBeenCalledWith('gx_user_difficulty', 'medium')

    // Manifest carries every settings key — next refresh's hydrate
    // fast-path will read them back without needing recovery.
    const manifest = JSON.parse(data.store.get(MANIFEST_KEY)!)
    expect(manifest).toContain('gx_user_sound_volume')
    expect(manifest).toContain('gx_user_music_volume')
    expect(manifest).toContain('gx_user_language')
    expect(manifest).toContain('gx_user_difficulty')
  })
  ...
  it('does NOT re-push settings keys that already match sdk.data', async () => {
    ...
    window.localStorage.setItem('gx_user_language', 'es')

    const data = makeFakeData({
      [MANIFEST_KEY]: JSON.stringify(['gx_user_language']),
      'gx_user_language': 'es'
    })
    const manager = new SaveManager(new CrazyGamesStrategy(() => data))
    await initWithFakeTimers(manager)
  ...
  it('on a fresh-remote hydrate, seeds sdk.data with every local payload key', async () => {
    ...
    window.localStorage.setItem(SAVE_KEYS.BEST_NODE, '5')
    window.localStorage.setItem(SAVE_KEYS.COINS, '300')
    window.localStorage.setItem('gx_user_language', 'fr')

    const data = makeFakeData()
    const manager = new SaveManager(new CrazyGamesStrategy(() => data))
    await initWithFakeTimers(manager)

    expect(data.setItem).toHaveBeenCalledWith(SAVE_KEYS.BEST_NODE, '5')
    expect(data.setItem).toHaveBeenCalledWith(SAVE_KEYS.COINS, '300')
    expect(data.setItem).toHaveBeenCalledWith('gx_user_language', 'fr')
  })
  ...
  it('cloud-only mode: payload writes never land in raw localStorage', async () => {
    ...
    window.localStorage.setItem(SAVE_KEYS.BEST_NODE, '7')
    window.localStorage.setItem(SAVE_KEYS.COINS, '500')
    window.localStorage.setItem('gx_user_sound_volume', '0.4')
    await vi.runAllTimersAsync()

    // sdk.data has the values.
    expect(data.store.get(SAVE_KEYS.BEST_NODE)).toBe('7')
    expect(data.store.get(SAVE_KEYS.COINS)).toBe('500')
    expect(data.store.get('gx_user_sound_volume')).toBe('0.4')

    // Raw localStorage is empty of every gameplay + bookkeeping key.
    expect(rawGet(SAVE_KEYS.BEST_NODE)).toBeNull()
    expect(rawGet(SAVE_KEYS.COINS)).toBeNull()
    expect(rawGet('gx_user_sound_volume')).toBeNull()
  ...
  it('cloud-only mode: scrubs pre-existing payload + bookkeeping at boot, preserves dev toggles', async () => {
    ...
    window.localStorage.setItem(SAVE_KEYS.BEST_NODE, '6')
    ...
    window.localStorage.setItem(MANIFEST_KEY, JSON.stringify([SAVE_KEYS.BEST_NODE, SAVE_KEYS.COINS, 'gx_user_sound_volume']))
    window.localStorage.setItem('fps', 'true')
    ...
    expect(window.localStorage.getItem(SAVE_KEYS.BEST_NODE)).toBe('6')
    expect(window.localStorage.getItem(SAVE_KEYS.COINS)).toBe('8818')
    expect(window.localStorage.getItem('gx_user_sound_volume')).toBe('0.7')
    expect(data.store.get(SAVE_KEYS.BEST_NODE)).toBe('6')
    expect(data.store.get('gx_user_sound_volume')).toBe('0.7')
  })
  ...
  it('cloud-only mode: saveDataVersion bumps AFTER patchLocalStorage, not during hydrate', async () => {
    ...
    const data = makeFakeData({
      [MANIFEST_KEY]: JSON.stringify([SAVE_KEYS.BEST_NODE]),
      [SAVE_KEYS.BEST_NODE]: '6'
    })
    const manager = new SaveManager(
      new CrazyGamesStrategy(() => data),
      window.localStorage,
  ...
  it('cloud-only mode: a write after refresh does NOT shrink the cloud manifest', async () => {
    ...
    expect(data.store.get('gx_user_language')).toBe('fr')
