...
/**
 * ─── `glyphyx_state` — the single persisted state object ───────────────────────
 *
 * EVERY persisted value glyphyx touches — meta progression, the resumable
 * run snapshot, user settings, retention bookkeeping — lives inside ONE
 * in-memory record (`glyphyxState`), and exactly ONE localStorage key is ever
 * written: `glyphyx_state`.
 *
 * Why one object:
 ...
 *     cloud payload is literally `{ glyphyx_state, __save_meta__ }`. One object
 ...
 * `gx_`-prefixed. They are a contract with the player base: renaming one
...
export const STATE_KEY = 'glyphyx_state'
 ...
 *  writes `glyphyx_state` to localStorage synchronously. Called from the
...
const buildInitial = (): Record<string, any> => {
  ...
  // Generic fold: if a player somehow has individual `gx_*` entries in raw
  ...
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k && k.startsWith('gx_')) stragglers.push(k)
    }
...
export const glyphyxState: Ref<Record<string, any>> = ref(buildInitial())
...
export const getState = <T = unknown>(key: string, fallback?: T): T => {
  const v = glyphyxState.value[key]
  return (v === undefined ? fallback : v) as T
}

export const hasState = (key: string): boolean => glyphyxState.value[key] !== undefined

export const setState = (key: string, value: unknown): void => {
  // Replace the record identity so `watch(glyphyxState)` (shallow) fires for every
  // consumer that mirrors a field into its own ref.
  glyphyxState.value = { ...glyphyxState.value, [key]: value }
  schedulePersist()
}
...
export const setStates = (patch: Record<string, unknown>): void => {
  glyphyxState.value = { ...glyphyxState.value, ...patch }
  schedulePersist()
}

export const removeState = (key: string): void => {
  if (glyphyxState.value[key] === undefined) return
  const next = { ...glyphyxState.value }
  delete next[key]
  glyphyxState.value = next
  schedulePersist()
}
...
export const reloadGlyphyxState = (): void => {
  glyphyxState.value = buildInitial()
}

/** Test-only: wipe both the in-memory blob and the persisted entry. */
export const __resetGlyphyxState = (): void => {
  glyphyxState.value = {}
