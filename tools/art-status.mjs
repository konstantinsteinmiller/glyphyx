 ...
 * play and useless at the bench, where the whole point is to know what is
 * still missing. Nothing here touches a browser: it reads the catalogue the
 * renderer probes and looks in `public/`.
 *
...
const { ART_CATALOGUE, artTarget } = await import(pathToFileURL(join(ROOT, 'src', 'game', 'artCatalogue.ts')).href)
