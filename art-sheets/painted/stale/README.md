...
| `sheet-runes-mage-orbglyph.jpg` | **Not broken — kept for its GLYPH** (2026-09-13). The first orb sheet that ever passed the slicer. Its stones are weaker than the roll that replaced them (smaller in the panel, a shallow muddy carve, and a stray painted grid line beside `mage-river-lv1` that the slicer did not erase), but its glyph is the RIGHT one: a solid core inside a ring with four sparks on the diagonals, as `glyphs.ts` draws it. The roll that shipped has better stones and a simplified glyph — a bare ring with a stem, no core, no sparks. Re-slice this one if the glyph matters more than the stone, or roll again asking for the core and the four sparks by name. |
